...
def weights_init(neuron_network):
    classname = neuron_network.__class__.__name__  # python trick that will look for the type of connection
    # in the object "neuron_network"
    # (convolution or full connection)
    if classname.find('Conv') != -1:  # if the connection is a convolution
        weight_shape = list(neuron_network.weight.data.size())  # list containing the shape of the weights
        # in the object "neuron_network"
        fan_in = np.prod(weight_shape[1:4])  # dim1 * dim2 * dim3
        fan_out = np.prod(weight_shape[2:4]) * weight_shape[0]  # dim0 * dim2 * dim3
        w_bound = np.sqrt(6. / (fan_in + fan_out))  # weight bound
        # generating some random weights of order inversely proportional
        neuron_network.weight.data.uniform_(-w_bound, w_bound)
        # to the size of the tensor of weights
        neuron_network.bias.data.fill_(0)  # initializing all the bias with zeros
    elif classname.find('Linear') != -1: # if the connection is a full connection
        # list containing the shape of the weights in the object "neuron_network"
        weight_shape = list(neuron_network.weight.data.size())
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space):
        ...
        self.lstm = nn.LSTMCell(32 * 3 * 3, 256)  # making an LSTM (Long Short Term Memory) to learn the temporal
        # properties of the input - we obtain a big encoded vector S of size 256 that encodes an event of the game
    ...
    def forward(self, inputs):
        ...
        x = F.elu(self.conv1(inputs))  # forward propagating the signal from the input images
        # to the 1st convolutional layer
        x = F.elu(self.conv2(x))  # forward propagating the signal from the 1st convolutional layer
        # to the 2nd convolutional layer
        x = F.elu(self.conv3(x))  # forward propagating the signal from the 2nd convolutional layer
        # to the 3rd convolutional layer
        x = F.elu(self.conv4(x))  # forward propagating the signal from the 3rd convolutional layer
        # to the 4th convolutional layer
        x = x.view(-1, 32 * 3 * 3)  # flattening the last convolutional layer into this 1D vector x
        hx, cx = self.lstm(x, (hx, cx))  # the LSTM takes as input x and the old hidden & cell states and ouputs
        # the new hidden & cell states
        x = hx # getting the useful output, which are the hidden states (principle of the LSTM)
        return self.critic_linear(x), self.actor_linear(x), (hx, cx)  # returning the output of the critic (V(S)),
        # the output of the actor (Q(S,A)), and the new hidden & cell states ((hx, cx))
